...
lemma permRealizable_of_mem_alternating_of_almost_generators {B : Board}
    {g c : Equiv.Perm (Fin B.tileCount)} {a b d : Fin B.tileCount}
    (hgReal : PermRealizable (B := B) g) (hcReal : PermRealizable (B := B) c)
  ...
  have hcmem : c ∈ Subgroup.closure S := by
    ...
    simp [S]
  have halt_le : alternatingGroup (Fin B.tileCount) ≤ Subgroup.closure S :=
    NPuzzle.Group.alternatingGroup_le_of_mem_almost_full_cycle_and_three_cycle
      hgmem hcmem hcycle hsupp hthree hca hcb hcd hfixed hgd hab hdb
  exact permRealizable_of_mem_closure (B := B) (S := S)
    (by
      intro τ hτ
      simp [S] at hτ
...
lemma reachable_goal_to_cfg_bottomRight_of_almost_generators {B : Board}
    {g c : Equiv.Perm (Fin B.tileCount)} {a b d : Fin B.tileCount}
    (hgReal : PermRealizable (B := B) g) (hcReal : PermRealizable (B := B) c)
    (hcycle : IsCycle g) (hsupp : g.support = Finset.univ.erase b)
    (hthree : IsThreeCycle c)
  ...
  obtain ⟨cfg', hbr', hreach, hperm⟩ :=
    permRealizable_of_mem_alternating_of_almost_generators
      hgReal hcReal hcycle hsupp hthree hca hcb hcd hfixed hgd hab hdb hσ
  have hcfg : cfg = cfg' := by
    apply config_eq_of_tileList_and_blank cfg cfg'
    · rw [hbr, hbr']
    · exact tileList_eq_of_tileListPerm_eq _ _
...
lemma reachable_goal_to_cfg_bottomRight_of_almost_parity_generators {B : Board}
    {g c : Equiv.Perm (Fin B.tileCount)} {a b d : Fin B.tileCount}
    (hgReal : PermRealizable (B := B) g) (hcReal : PermRealizable (B := B) c)
    (hcycle : IsCycle g) (hsupp : g.support = Finset.univ.erase b)
    (hthree : IsThreeCycle c)
    ...
    (hpar : parityClass cfg = targetParity B) :
    Reachable (goal B) cfg :=
  reachable_goal_to_cfg_bottomRight_of_almost_generators
    hgReal hcReal hcycle hsupp hthree hca hcb hcd hfixed hgd hab hdb cfg hbr
    (invStat_even_of_parity_bottomRight cfg hbr hpar)
...
lemma tiles_to_goal_bottomRight_of_almost_parity_generators {B : Board}
    {g c : Equiv.Perm (Fin B.tileCount)} {a b d : Fin B.tileCount}
    (hgReal : PermRealizable (B := B) g) (hcReal : PermRealizable (B := B) c)
    (hcycle : IsCycle g) (hsupp : g.support = Finset.univ.erase b)
    (hthree : IsThreeCycle c)
  ...
  reachable_symm
    (reachable_goal_to_cfg_bottomRight_of_almost_parity_generators
      hgReal hcReal hcycle hsupp hthree hca hcb hcd hfixed hgd hab hdb cfg hbr hpar)
