...
| **R6** | Start rectangular core | ✅ necessity + abstract sufficiency tail done | `NPuzzle/Rect/Basic.lean`, `Config.lean`, `TileInverse.lean`, `Reach.lean`, `PathEffect.lean`, `Corner.lean`, `CornerPerm.lean`, `CornerRealizable.lean`, `FullCyclePerm.lean`, `Parity.lean`, `Invariant.lean`, `TileGlue.lean`, `TilePerm.lean`, `TileSorted.lean`, `TileSign.lean`, `TileRelabel.lean`, `Realizable.lean`, `AbstractSufficiency.lean`, `GeneratorSufficiency.lean`: board geometry with directional adjacency, moves, reversible reachability, blank grid paths with visited-vertex lists/concrete runner/composition/reversal and arbitrary-target connectivity, cell-level path effect by repeated swaps, concrete bottom-right 2×2 loop with local cell effects and tile-list indices, its abstract `Fin B.tileCount` 3-cycle permutation and realized corner generator, an abstract compatible full-cycle permutation, README parity statistic, parity invariance, `tileList`+blank ⇒ config, valid tile-list ⇒ config/`Fin B.tileCount` permutation, sorted tile-list ⇒ goal permutation, sign ↔ inversion parity, relabel transports reachability/perms, realized perms form subgroup, full-cycle+3-cycle ⇒ bottom-right sufficiency, named conditional bottom-right sufficiency from the compatible full cycle |

`TileReach` / `TileConnectivity` / `Sufficiency` are green for 4×4, but still heavy and 4×4-specific. The first geometry-free sufficiency component is now `NPuzzle/Group/CycleThree.lean`; the first board-generic surface is `NPuzzle.Rect` (`Basic`, `Config`, `TileInverse`, `Reach`, `PathEffect`, `Corner`, `CornerPerm`, `CornerRealizable`, `FullCyclePerm`, `Parity`, `Invariant`, `TileGlue`, `TilePerm`, `TileSorted`, `TileSign`, `TileRelabel`, `Realizable`, `AbstractSufficiency`, `GeneratorSufficiency`). Rectangular necessity/parity invariance, directional adjacency, reversible reachability, blank path lifting/composition/connectivity, cell-level blank-path effects with a list-based normal form, concrete bottom-right 2×2 loop geometry with local cell effects and tile-list indices, its abstract and realized three-cycle permutation, an abstract compatible full-cycle permutation, `tileList` glue, valid tile-list packaging, the bottom-right tile-list permutation bridge, sorted-list identity, sign/inversion parity equivalence, relabel transport, realizable-permutation subgroup algebra, the conditional sufficiency tail, and the named full-cycle-to-bottom-right-sufficiency bridge are now discharged; the remaining generalization work is realizing the full-cycle generator by rectangular-grid slides.
