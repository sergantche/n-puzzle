...
| **R6** | Start rectangular core | ✅ necessity + tile-list/permutation/sign package done | `NPuzzle/Rect/Basic.lean`, `Config.lean`, `Parity.lean`, `Invariant.lean`, `TileGlue.lean`, `TilePerm.lean`, `TileSorted.lean`, `TileSign.lean`: board geometry, moves, README parity statistic, parity invariance, `tileList`+blank ⇒ config, valid tile-list ⇒ config/`Fin B.tileCount` permutation, sorted tile-list ⇒ goal permutation, sign ↔ inversion parity |

`TileReach` / `TileConnectivity` / `Sufficiency` are green for 4×4, but still heavy and 4×4-specific. The first geometry-free sufficiency component is now `NPuzzle/Group/CycleThree.lean`; the first board-generic surface is `NPuzzle.Rect` (`Basic`, `Config`, `Parity`, `Invariant`, `TileGlue`, `TilePerm`, `TileSorted`, `TileSign`). Rectangular necessity/parity invariance, `tileList` glue, valid tile-list packaging, the bottom-right tile-list permutation bridge, sorted-list identity, and sign/inversion parity equivalence are now discharged; the remaining generalization work is rectangular-grid analogues of the full cycle and compatible 3-cycle macros for sufficiency.
