...
| **R6** | Start rectangular core | ✅ necessity + abstract sufficiency tail done | `NPuzzle/Rect/Basic.lean`, `Config.lean`, `TileInverse.lean`, `Reach.lean`, `Corner.lean`, `CornerPerm.lean`, `Parity.lean`, `Invariant.lean`, `TileGlue.lean`, `TilePerm.lean`, `TileSorted.lean`, `TileSign.lean`, `TileRelabel.lean`, `Realizable.lean`, `AbstractSufficiency.lean`: board geometry with directional adjacency, moves, reversible reachability, blank grid paths with composition/reversal and arbitrary-target connectivity, concrete bottom-right 2×2 loop with local cell effects and tile-list indices, its abstract `Fin B.tileCount` 3-cycle permutation, README parity statistic, parity invariance, `tileList`+blank ⇒ config, valid tile-list ⇒ config/`Fin B.tileCount` permutation, sorted tile-list ⇒ goal permutation, sign ↔ inversion parity, relabel transports reachability/perms, realized perms form subgroup, full-cycle+3-cycle ⇒ bottom-right sufficiency |

`TileReach` / `TileConnectivity` / `Sufficiency` are green for 4×4, but still heavy and 4×4-specific. The first geometry-free sufficiency component is now `NPuzzle/Group/CycleThree.lean`; the first board-generic surface is `NPuzzle.Rect` (`Basic`, `Config`, `TileInverse`, `Reach`, `Corner`, `CornerPerm`, `Parity`, `Invariant`, `TileGlue`, `TilePerm`, `TileSorted`, `TileSign`, `TileRelabel`, `Realizable`, `AbstractSufficiency`). Rectangular necessity/parity invariance, directional adjacency, reversible reachability, blank path lifting/composition/connectivity, concrete bottom-right 2×2 loop geometry with local cell effects and tile-list indices, its abstract three-cycle permutation, `tileList` glue, valid tile-list packaging, the bottom-right tile-list permutation bridge, sorted-list identity, sign/inversion parity equivalence, relabel transport, realizable-permutation subgroup algebra, and the conditional sufficiency tail are now discharged; the remaining generalization work is rectangular-grid analogues of the full cycle and compatible 3-cycle realization.
