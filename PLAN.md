...
`TileReach` / `TileConnectivity` / `Sufficiency` are green for 4×4, but still heavy and 4×4-specific. The first geometry-free sufficiency component is now `NPuzzle/Group/CycleThree.lean`; the first board-generic surface is `NPuzzle.Rect` (`Basic`, `Config`, `TileInverse`, `Reach`, `PathEffect`, `Corner`, `CornerPerm`, `CornerRealizable`, `FullCyclePerm`, `Parity`, `Invariant`, `TileGlue`, `TilePerm`, `TileSorted`, `TileSign`, `TileRelabel`, `Realizable`, `AbstractSufficiency`, `GeneratorSufficiency`). Rectangular necessity/parity invariance, directional adjacency, reversible reachability, blank path lifting/composition/connectivity, cell-level blank-path effects, concrete bottom-right 2×2 loop geometry with local cell effects and tile-list indices, its abstract and realized three-cycle permutation, an abstract compatible full-cycle permutation, `tileList` glue, valid tile-list packaging, the bottom-right tile-list permutation bridge, sorted-list identity, sign/inversion parity equivalence, relabel transport, realizable-permutation subgroup algebra, the conditional sufficiency tail, and the named full-cycle-to-bottom-right-sufficiency bridge are now discharged; the remaining generalization work is realizing the full-cycle generator by rectangular-grid slides.
